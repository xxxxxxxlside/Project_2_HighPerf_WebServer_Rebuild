...
本项目基于 `docs/mvp2.md` 与 `docs/RouteC_V23_9.md` 的要求，当前推进到 **Week1 Day5**：在 Day4 的动态缓冲区基础上，加入最小 HTTP 请求头解析和协议边界防御。
...
- `Server` 负责监听 socket 初始化、切换非阻塞、注册 epoll、维护最小连接表、读取数据、切分完整请求头、执行 Day5 解析和防御逻辑。
- `socket_utils` 负责封装 `socket`、`setsockopt(SO_REUSEADDR)`、`bind`、`listen`、`fcntl(O_NONBLOCK)`、`accept4` 和小响应的 best-effort 写回。
- `dynamic_buffer` 负责在应用层暂存字节流，并按 `\r\n\r\n` 找出完整请求头边界。
- `http_request` / `http_parser` / `http_response` 负责最小请求头解析与错误响应拼装。
- `epoll_poller` 负责封装 `epoll_create1`、`epoll_ctl`、`epoll_wait`。
- 当前只做到 Day5，不提前进入 Day6 的 `200 OK` 正常响应写回和 outbuf 管理。
...
- 已完成：工程初始化、模块拆分、监听启动、非阻塞监听 socket、基础 accept 循环、Epoll LT 监听事件循环、应用层动态缓冲区、按 `\r\n\r\n` 处理 TCP 粘包 / 拆包、Request Line / Header 解析、`Content-Length` 提取、431 / 411 / 501 防御。
- 未实现：Day6 的 `200 OK` 正常响应写回、outbuf、EPOLLOUT 持续写、Week2 的 ReadyQueue 和连接生命周期治理。

当前版本的目标仍然是保持简单、清晰、方便 review，只推进到文档要求的 **Week1 Day5**。
