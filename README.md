...
本项目基于 `docs/mvp2.md` 与 `docs/RouteC_V23_9.md` 的要求，当前推进到 **Week1 Day4**：在 Day3 的 Epoll LT 事件循环基础上，引入最小应用层动态缓冲区，并按 `\r\n\r\n` 处理 TCP 粘包 / 拆包。
...
- `Server` 负责监听 socket 初始化、切换非阻塞、注册 epoll、维护最小连接表、读取数据并切分完整请求头。
...
- 当前只做到 Day4，不提前进入 Day5 的完整 HTTP 解析和 Day6 的响应写回。
...
- 未实现：Day5 的完整 HTTP 解析、Day6 的响应写回、Week2 的连接生命周期治理。

当前版本的目标仍然是保持简单、清晰、方便 review，只推进到文档要求的 **Week1 Day4**。
